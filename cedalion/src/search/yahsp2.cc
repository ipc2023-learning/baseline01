...
State *YAHSP2::compute_node(const State &state,
    ...
    SearchNode lookahead_node = ssp.get_node(lookahead_state);

    int state_g = ssp.get_node(state).get_real_g();
    ...
    SearchNode node = ssp.get_node(state);
...
void YAHSP2::trace_path(vector<const Operator *> &path, const State &state, SearchSpace &ssp) {
    ...
    while (true) {
        const SearchNodeInfo &info = ssp.get_node(
            g_state_registry->lookup_state(current_state_id)).get_info();
